...
const (
	...
	// OwnershipAdopted means this controller explicitly claimed a verified
	// existing resource and may manage its lifecycle.
...
// AgentCreationState records an in-flight or ambiguous Harness Agent create.
// +kubebuilder:validation:Enum=Pending;OutcomeUnknown
type AgentCreationState string

const (
	// AgentCreationPending means create intent is durable but ownership has not
	// yet been verified.
	AgentCreationPending AgentCreationState = "Pending"
	// AgentCreationOutcomeUnknown means the create request may have committed.
	AgentCreationOutcomeUnknown AgentCreationState = "OutcomeUnknown"
)
...
type HarnessGitopsAgentStatus struct {
	// CreationState records durable create intent until the Harness Agent is
	// verified as owned by this exact Kubernetes resource.
	// +optional
	CreationState AgentCreationState `json:"creationState,omitempty"`
...
// +kubebuilder:resource:shortName=hga
